...
The decompiler is a **conservative first pass**:

- **Done and round-trip-verified:** expression reconstruction (with the
  inline-immutable-leaves-only rule — a correct temp beats a wrong inline), arithmetic,
  concatenation, comparisons, table reads/writes, field access, method calls
  (`NAMECALL → obj:m(args)`), calls, multiple returns, varargs, globals/imports/upvalues,
  and nested closures. Straight-line functions recompile with the real compiler.
- **Not yet native, rendered honestly instead:** structured recovery of
  `if`/`while`/`repeat`/generic-`for` is incomplete. Until it lands, those regions are
  emitted with `if … then goto … end` / `::label::` and a comment, and the proto is flagged
  `partial: true`. Numeric-`for` setup is annotated. This output reflects the real control
  flow rather than guessing — but a proto marked `partial` is not guaranteed to recompile.
- Names come from debug info when present (a register is named after its debug local only
  when that is unambiguous); otherwise stable synthesized names (`pN` for params, `vN` for
  locals) are used.
...
The rule catalog (require/modules, services & instances, method results, datatype
constructors, signals, `#`/length, etc.) was generated by a multi-agent brainstorm and is
kept in [reference/decompiler-heuristics.json](reference/decompiler-heuristics.json) along
with the correctness pitfalls the implementation respects.
